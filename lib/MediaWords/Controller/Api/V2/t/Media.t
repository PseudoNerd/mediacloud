...
sub test_for_scraped_feeds($$)
{
    ...
    for my $site ( @{ $sites } )
    {
        ...
        MediaWords::Test::URLs::is_urls( $feed->{ url }, $site->{ feed_url }, "$site->{ name } feed url" );
    }
}
...
sub test_media_create_update($$)
{
    ...
    for my $site ( @{ $sites } )
    {
        ...
        MediaWords::Test::URLs::is_urls(
            $feeds->[ 0 ]->{ url },
            $site->{ feed_url },
            "media/create update $site->{ name } default feed url"
        );
        MediaWords::Test::URLs::is_urls(
            $feeds->[ 1 ]->{ url },
            $site->{ custom_feed_url },
            "media/create update $site->{ name } custom feed url"
        );
...
sub test_media_update($$)
{
    ...
    my $r = MediaWords::Test::API::test_put( '/api/v2/media/update',
        { media_id => $medium->{ media_id }, name => "$medium->{ name } FOO" } );
...
sub test_media_create($)
{
    ...
    MediaWords::Test::URLs::is_urls( $r->[ 0 ]->{ url }, $first_site->{ url }, "media/create url url" );
    ...
    MediaWords::Test::URLs::is_urls( $r->[ 0 ]->{ url }, $first_site->{ url }, "media/create existing url" );
...
sub test_media_suggestions_mark($)
{
    ...
    MediaWords::Test::API::test_put( '/api/v2/media/mark_suggestion',
        { media_suggestions_id => 0, status => 'approved' }, 1 );
    MediaWords::Test::API::test_put( '/api/v2/media/mark_suggestion',
        { media_suggestions_id => $ms_id, status => 'invalid_status' }, 1 );
